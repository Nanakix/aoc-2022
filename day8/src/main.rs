...
fn scenic_score(forest: &Vec<Vec<char>>, x: usize, y: usize, tree: &char) -> u32 {
    ...
    for (i, t) in forest[0..x].iter().rev().enumerate().map(|(index, row)| (index, row[y])) {
        if &t >= tree && !met {
            met = true;
            // println!("west: i: {}, x: {}", i, x);
            north_score = (i + 1) as u32;
        }
    }
    if !met { // no taller tree is at the west
        // println!("no taller tree is at the north");
    ...
    for (i, t) in forest[x+1..width].iter().enumerate().map(|(index, row)| (index, row[y])) {
        if &t >= tree && !met {
            met = true;
            // println!("eastern trees of t{}: {}", tree, t);
            // println!("east: t: {}, tree: {}, x:{}, i:{}",t, tree, x, i);
            // println!("east: i: {}, x: {}", i, x);
            south_score = (i + 1) as u32; //(x  as i32 - (x+i) as i32).abs() as u32;
        }
    }
    if !met { // no taller tree is at the east
        // println!("no taller tree is at the south");
    ...
    for (i, t) in forest[x][0..y].iter().rev().enumerate() {
        // println!("western trees of t{}: {}", tree, t);
        if t >= tree  && !met{
            met = true;
            // println!("west: i: {}, x: {}, y: {}", i, x, y);
            // if i as u32 == 0 { // corner case
                west_score = (i + 1) as u32;
            // }
            // else {
                // west_score = (y as i32 - i as i32).abs() as u32;
            // }
        }
    }
    
    if !met { // no taller tree is at the west
        // println!("no taller tree is at the west");
        west_score = y as u32;
    }
    else {
        met = false;
    ...
    for (i, t) in forest[x][y+1..width].iter().enumerate() {
        ...
        if t >= tree && !met {
            met = true;
            // println!("east: i: {}, x:{}, y: {}", i, x, y);
            east_score = (i + 1) as u32;
        }
    }
    if !met { // no taller tree is at the east
        // println!("no taller tree is at the east");
        east_score = (width as i32 - (y+1) as i32).abs() as u32;
    }
    // println!("{}", tree);
    // println!("n:{} w:{} s:{} e:{}", north_score, west_score, south_score, east_score);
