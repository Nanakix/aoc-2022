...
fn check_column(forest: &Vec<Vec<char>>,  x: usize, y: usize, tree: &char) -> bool {
    ...
    if forest[0..x+1].iter().map(|row| row[y]).any(|e| e >= *tree) {
    ...
    if forest[x][0..y].iter().any(|e| e >= tree) {
        println!("rip2 petit arbre {}, tu n'es pas le plus grand de ta colonne en partant vers le haut", tree);
        side -= 1;
    }
    if forest[x][y+1..width].iter().any(|e| e >= tree) {
        println!("rip2 petit arbre {}, tu n'es pas le plus grand de ta ligne en partant vers le bas", tree);
        side -= 1;
    }
    side != 0
}
